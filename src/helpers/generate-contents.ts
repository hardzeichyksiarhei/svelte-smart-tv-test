...
export const generateContents = (rows = 10, columns = 25) => {
    return [...new Array(rows)].map<Rows>((_, i) => ({
        id: `row-${i}`,
        duration: i % 2 === 0 ? 800 : 400,
        contents: [...new Array(columns)].map((_, j) => ({
            title: `Title ${i}_${j}`,
            image: `https://random.imagecdn.app/300/200?slider-${i}_${j}`,
